...
class CalendarSummaryService
{
    public function getDailySummaries()
    {
        ...
        $aiRun = AiRun::where("run_type", "daily")
    ...
    public function Summary()
    {
        ...
        $teamUsersWithEvents = User::whereHas("teamUser")
            ...
            ->with([
                "teamUser",
                "events" => function ($query) use ($todayDate) {
                    $query
        ...
        if ($teamUsersWithEvents->isEmpty()) {
             AiRun::create([
                "entity_type" => "TeamCalendar",

                "entity_id" => 0,

                "run_type" => "daily",

                "input_data" => ["prompt" => "No events scheduled for today."],

                "output_data" => ["message" => "No events scheduled for today."],
        ...
        foreach ($teamUsersWithEvents as $user) {
            ...
            $userContext = $this->buildEventPrompt(
                $todayDate,
                $user->events,
                $user->teamUser,
                $user,
        ...
        try {
            ...
            // 4. Save AI run to database

            AiRun::create([
                "entity_type" => "TeamCalendar", // arbitrary label

                "entity_id" => 0, // 0 because it's batch, not single user

                "run_type" => "daily", // could also be 'trigger' if event-based

                "input_data" => ["prompt" => $megaPromptData],
            ...
            return response()->json(
                [
                    "status" => "error",
                    "message" => "AI failed to process batch",
                ],
                500
            );
        }
    ...
    private function analyzeEvents($events)
    {
        ...
        foreach ($events as $event) {
            ...
            if (
                in_array($event->category, $analysis["work_categories"]) ||
                $event->category === "Work"
            ) {
                $analysis["work_event_count"]++;
            } elseif (
                in_array($event->category, $analysis["personal_categories"])
            ) {
    ...
    private function buildEventPrompt(
        $date,
        $events,
        $teamUser,
        $user,
        ...
        foreach ($events as $index => $event) {
            ...
            $duration = Carbon::parse($event->start_hour)->diffInMinutes(
                Carbon::parse($event->end_hour)
            );
            ...
            $content .=
                "\n" .
                ($index + 1) .
                ". Event Duration: {$duration} minutes{$urgencyFlag}\n";
            ...
            if ($otherGuests->isNotEmpty()) {
                $attendeesList = $otherGuests
                    ->map(function ($guest) {
                        $role = $guest->teamUser
                            ? " ({$guest->teamUser->poste})"
                            : "";
